...
internal sealed partial class MCPPickerView : BaseModelPickerView
{
    ...
    public override async Task Load(List<ModelType> types)
    {
        ...
        var mcpModels = await MCPModelProvider.Instance.GetModelsAsync(ignoreCached: true) ?? [];
        mcpModels.ToList().ForEach(models.Add);
        VisualStateManager.GoToState(this, "ShowModels", true);
    }

    private void MCPCopyUrl_Click(object sender, RoutedEventArgs e)
    {
    ...
    private void MCPViewModelDetails_Click(object sender, RoutedEventArgs e)
    {
        if (sender is MenuFlyoutItem btn && btn.Tag is ModelDetails details)
        {
