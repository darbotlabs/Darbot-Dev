...
internal sealed partial class OmniparserPickerView : BaseModelPickerView
{
    private ObservableCollection<ModelDetails> models = new ObservableCollection<ModelDetails>();

    public OmniparserPickerView()
    {
    ...
    public override async Task Load(List<ModelType> types)
    {
        ...
        var omniparserModels = await OmniparserModelProvider.Instance.GetModelsAsync(ignoreCached: true) ?? [];
        omniparserModels.ToList().ForEach(models.Add);
        VisualStateManager.GoToState(this, "ShowModels", true);
    }

    private void OmniparserCopyUrl_Click(object sender, RoutedEventArgs e)
    {
    ...
    private void OmniparserViewModelDetails_Click(object sender, RoutedEventArgs e)
    {
        if (sender is MenuFlyoutItem btn && btn.Tag is ModelDetails details)
        {
